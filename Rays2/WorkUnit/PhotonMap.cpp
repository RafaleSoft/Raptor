...
CPhotonMap::CPhotonMap(unsigned int minsize):nbPhotons(0)
{
	if (minsize < 1)
		photonsPerLight = 1;
	else
...
void CPhotonMap::DumpPhotons(const char* name)
{
	char	buffer[256] ;
	long	newcolor = 0;
	FILE	*out;
...
void CPhotonMap::GetDensity(const CGenericVector<float> position, CWVector& target) const
{
	//	Find nearest X
	...
	while ((step > 0) && (deltax < DELTA_PHOTON_DENSITY))
	{
		if (DELTA_PHOTON_DENSITY > ABS(m_pPhotons[step].position[1] - position.Y()))
			if (DELTA_PHOTON_DENSITY > ABS(m_pPhotons[step].position[2] - position.Z()))
			{
				...
				R += (long)(scale * m_pPhotons[step].energy[0]);
				G += (long)(scale * m_pPhotons[step].energy[1]);
				B += (long)(scale * m_pPhotons[step].energy[2]);
			}
		step--;
	...
	target.X(MIN(65535,R >> 8));
	target.Y(MIN(65535,G >> 8));
	target.Z(MIN(65535,B >> 8));
}
...
void CPhotonMap::BuildPhotonMap(void)
{
	...
	for (i=0;i<photonsPerLight;i++)
	{
		...
		COUT << ".Photon " << i << ": " << nb << endl;
		neighbours.RemoveAll();
	}
}

void CPhotonMap::Hit( LPRAYTRACERDATA World, CGenericRay& lightRay,CWVector& sourceColor )
{
	int level = lightRay.level;
	...
	if ( (numSurfaces = World->objects.GetSize()) > 0 )
	{
		// ========================== INTERSECTION ==========================
		...
		if ( lightRay.t < HUGE_REAL )
		{
			...
			lightRay.hit = lightRay.origin + (lightRay.t * lightRay.direction) ;
			lightRay.hit.H(1);

			local_color = lightRay.surface->GetLocalColor( lightRay );
			local_color.X(local_color.X() * lightRay.surface->shading.diffuse);
			local_color.Y(local_color.Y() * lightRay.surface->shading.diffuse);
			local_color.Z(local_color.Z() * lightRay.surface->shading.diffuse);
			...
			if ((oldSurface != lightRay.surface) && (level > 1))
			{
				...
				memcpy(&p.energy,sourceColor.vector(),4*sizeof(short));
				p.nb_neighbours = 0;
				p.pNeighbours = NULL;
				AddPhoton(p);
			}
			...
			if (level <= World->camera.reflection_depth)
			{
				// --- compute the normal ---
				...
				reflected_ray.direction = (-tmp - tmp) * lightRay.normal;
				...
				CWVector tmpColor;
				tmpColor.X(local_color.X() * lightRay.surface->shading.reflection);
				tmpColor.Y(local_color.Y() * lightRay.surface->shading.reflection);
				tmpColor.Z(local_color.Z() * lightRay.surface->shading.reflection);
			...
			if (level <= World->camera.refraction_depth)
			{
				...
				if (costheta1 >= 0.0 )
				{
					...
					if (cos(asin(n2n1))<=costheta1)
					{
						...
						refracted_ray.direction = coef * lightRay.normal + n1n2 * lightRay.direction;
					...
					{
						refracted_ray.fact = lightRay.surface->shading.refraction * lightRay.fact ;
						refracted_ray.direction = ((-2.0f*costheta1) * (lightRay.normal));
				...
				else
				{
					...
					refracted_ray.fact = lightRay.surface->shading.refraction * lightRay.fact ;
					refracted_ray.direction = (coef * lightRay.normal);
					refracted_ray.direction += (n1n2 * lightRay.direction);
					refracted_ray.direction.Normalize();
				}
				...
				CWVector tmpColor;
				tmpColor.X(local_color.X() * lightRay.surface->shading.refraction);
				tmpColor.Y(local_color.Y() * lightRay.surface->shading.refraction);
				tmpColor.Z(local_color.Z() * lightRay.surface->shading.refraction);
