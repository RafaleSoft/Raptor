...
CContextManager::RENDERING_CONTEXT_ID  CWin32ContextManager::glCreateExtendedContext(const RAPTOR_HANDLE& device,int displayMode,bool global)
{
	...
	void CWin32ContextManager::glBindPBuffer(PIXEL_BUFFER_ID pbuffer,unsigned int selectBuffer)
	{
		if (pBuffers[pbuffer].pbuffer == NULL)
		{
			stringstream str;
        ...
        if ((selectBuffer >= CGL_CUBEMAP_PX) && (selectBuffer <= CGL_CUBEMAP_NZ))
        {
            const CRaptorExtensions *const pExtensions = this->glGetExtensions();
        
            int piAttribList[3];
            piAttribList[0] = WGL_CUBE_MAP_FACE_ARB;
            piAttribList[1] = ((selectBuffer - CGL_CUBEMAP_PX)  >> 4 )+ WGL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB;
            piAttribList[2] = 0;
