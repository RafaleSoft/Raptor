	...
	:m_name(task.m_name)
{
	...
	{
		CProgramParameters::ParameterBase *param = task.m_parameters[i];
		if (param->isA((cl_mem)0))
			::clRetainMemObject(((CProgramParameters::Parameter<cl_mem>*)param)->p);
...
CRaptorComputeTask::~CRaptorComputeTask(void)
{
	...
	{
		CProgramParameters::ParameterBase* param = m_parameters[0];
		if (NULL != param)
		{
			m_parameters.erase(m_parameters.begin());
			if (param->isA((cl_mem)0))
				::clReleaseMemObject(((CProgramParameters::Parameter<cl_mem>*)param)->p);
...
void CRaptorComputeTask::setParameter(size_t pos,const CRaptorComputeMemory::IBufferObject &bo)
{
	if (pos < m_parameters.size())
	{
		...
		if (NULL != cb)
		{
			...
			{
				CProgramParameters::Parameter<cl_mem>* param = (CProgramParameters:: Parameter<cl_mem>*)m_parameters[pos];
				cl_int res = ::clReleaseMemObject(param->p);
				res = ::clRetainMemObject(buffer);

				((CProgramParameters::Parameter<cl_mem>*)m_parameters[pos])->p = buffer;
			}
		}
	}
}
