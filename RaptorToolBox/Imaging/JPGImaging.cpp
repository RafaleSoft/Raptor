...
bool CJPGImaging::storeImageFile(const std::string& fname,CTextureObject* const T)
{
    if (T == NULL)
        return false;
    ...
    cinfo.image_height = T->getHeight();
    cinfo.input_components = 3;
    cinfo.in_color_space = JCS_RGB;
    jpeg_set_defaults(&cinfo);
	
	jpeg_start_compress(&cinfo,TRUE);
    int row_stride = T->getWidth() * 4;
...
bool CJPGImaging::loadImageFile(const std::string& fname,CTextureObject* const T)
{
    if (T==NULL)
        return false;
    ...
    T->setSize(cinfo.image_width,cinfo.image_height);
    T->allocateTexels();
	JSAMPLE *image_buffer = T->getTexels();
