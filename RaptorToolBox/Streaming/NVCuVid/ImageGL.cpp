...
GLuint compile_glsl_shader(GLenum program_type, const char *code)
{
	...
	pExtensions->glBindProgramARB(program_type, program_id);
	pExtensions->glProgramStringARB(program_type, GL_PROGRAM_FORMAT_ASCII_ARB, (GLsizei) strlen(code), (GLubyte *) code);
    ...
    , bIsCudaResource_(false)
{
    ...
    {
		pExtensions->glBindBufferARB(GL_PIXEL_UNPACK_BUFFER_ARB, gl_pbo_[n]);
		pExtensions->glBufferDataARB(GL_PIXEL_UNPACK_BUFFER_ARB, nTexWidth*nTexHeight*4, NULL, GL_STREAM_DRAW_ARB);
		pExtensions->glBindBufferARB(GL_PIXEL_UNPACK_BUFFER_ARB, 0);
...
const
{
    // Common display code path
    {
		...
		pExtensions->glBindProgramARB(GL_FRAGMENT_PROGRAM_ARB, gl_shader_);
